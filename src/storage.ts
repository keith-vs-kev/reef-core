...
export class SqliteSessionStore implements SessionStore {
  ...
  getAll(): SessionRow[] {
    const rows = this.db.prepare('SELECT * FROM sessions ORDER BY created_at DESC').all() as SessionDbRow[];
  ...
  getOutput(id: string): string[] {
    const rows = this.db.prepare(
      'SELECT content FROM output_lines WHERE session_id = ? ORDER BY id'
